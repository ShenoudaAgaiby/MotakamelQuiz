...
function StudentDashboard({ user, onLogout }) {
    ...
    const [activeQuiz, setActiveQuiz] = useState(null) // For legacy global quizzes
    ...
    const fetchQuizzes = async () => {
        ...
        try {
            // Fetch Global Questions (school_id is null) for the student's grade
            const { data: qData, error: qError } = await supabase
                .from('questions')
                .select('*, subjects!fk_questions_subjects(master_subjects!master_subject_id(name)), grades!fk_questions_grades(name)')
                .is('school_id', null)
                .eq('grade_id', user.grade_id)

            if (qError) {
                console.error('StudentDashboard Supabase Error:', qError)
                throw qError
            }

            // Group by subject for the UI
            const grouped = qData.reduce((acc, q) => {
                const subjectName = q.subjects?.master_subjects?.name || 'عام'
                if (!acc[subjectName]) acc[subjectName] = []
                acc[subjectName].push(q)
                return acc
            }, {})

            setQuizzes(Object.keys(grouped).map(subject => ({
                subject,
                questions: grouped[subject],
                difficulty: grouped[subject][0].difficulty
            })))
        } catch (err) {
    ...
    const fetchCompetitions = async () => {
        if (!supabase || !user.grade_id) return
        try {
        ...
        try {
            ...
            difficulties.forEach(({ key, quota }) => {
                ...
                let pool = allAvailableQuestions.filter(q => {
                    ...
                    return q.difficulty === key &&
                        week >= comp.start_week &&
                        week <= comp.end_week &&
                        !alreadySeenIds.has(q.id); // ✅ Only get unseen questions
                })
                ...
                if (pool.length >= quota) {
                    ...
                    const seenPool = allAvailableQuestions.filter(q => {
                        ...
                        return q.difficulty === key &&
                            week >= comp.start_week &&
                            week <= comp.end_week &&
                            alreadySeenIds.has(q.id); // Only get seen questions
                    })
            ...
            <div className="max-w-4xl w-full">
                <header className="flex justify-between items-center mb-8 glass-card p-6 rounded-2xl">
                    <div>
                        ...
                        <p className="text-slate-600">أهلاً بك، {user.name} | مدرسة {user.schoolName || 'النموذجية'}</p>
                    </div>
                    <button
                        onClick={onLogout}
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    ...
                    <div className="glass-card p-6 rounded-2xl text-center">
                        <div className="text-4xl mb-4">📊</div>
                        <h3 className="font-bold mb-1 text-2xl text-brand-primary">{quizzes.length}</h3>
                                    ...
                                    <div className="flex gap-4 items-center">
                                        ...
                                        <div>
                                            ...
                                            <div className="flex gap-2 items-center mt-1">
                                                ...
                                                <span className="text-[10px] bg-blue-50 px-2 py-0.5 rounded-full font-bold text-blue-600">الأسبوع {comp.week}</span>
                ...
                <section className="glass-card rounded-2xl overflow-hidden mb-8">
                    <div className="p-6 border-b border-slate-100 bg-white/50">
                        <h2 className="text-xl font-bold">بنك الأسئلة المفتوح (تدريب حر)</h2>
                    </div>
                    <div className="divide-y divide-slate-100">
                        {loading ? (
                            <div className="p-12 text-center text-slate-400">جاري تحميل الأسئلة...</div>
                        ) : quizzes.length > 0 ? (
                            quizzes.map((quiz, index) => (
                                <div key={index} className="p-6 flex items-center justify-between hover:bg-slate-50 transition-all">
                                    <div className="flex gap-4 items-center">
                                        <div className="w-12 h-12 bg-blue-50 text-blue-600 rounded-xl flex items-center justify-center font-bold text-xl">
                                            {quiz.subject ? quiz.subject[0] : '?'}
                                        </div>
                                        <div>
                                            <h4 className="font-bold text-slate-800">{quiz.subject}</h4>
                                            <p className="text-xs text-slate-400">{quiz.questions.length} أسئلة • تدريب غير محدود</p>
                                        </div>
                                    </div>
                                    <button
                                        onClick={() => handleStartQuiz(quiz)}
                                        className="px-6 py-2 bg-brand-primary text-white rounded-xl font-bold text-sm shadow-md hover:scale-105 transition-all"
                                    >
                                        ابدأ التدريب
                                    </button>
                                </div>
                            ...
                            <div className="p-12 text-center text-slate-400 italic">لا توجد أسئلة تدريبية حالياً.</div>
                        )}
                    </div>
                </section>
